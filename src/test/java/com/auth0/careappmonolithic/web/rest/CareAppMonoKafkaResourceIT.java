...
import com.auth0.careappmonolithic.IntegrationTest;
import com.auth0.careappmonolithic.config.EmbeddedKafka;
import com.auth0.careappmonolithic.config.KafkaSseConsumer;
import com.auth0.careappmonolithic.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class CareAppMonoKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/care-app-mono-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/care-app-mono-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/care-app-mono-kafka/unregister"));
                return;
            }
        }
